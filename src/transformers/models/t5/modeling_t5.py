...
@add_start_docstrings(
    """
    The full T5 Model transformer with a sequence classification head on top (linear layer).
    """,
    T5_START_DOCSTRING,
)
class T5ForSequenceClassification(T5PreTrainedModel):
    ...
    def forward(
        ...
        hidden_states = torch.cat([outputs[0],outputs[2]],dim=-1)
        ...
        return SequenceClassifierOutputWithPast(
            ...
            hidden_states=None if outputs.encoder_hidden_states is None else outputs.encoder_hidden_states+outputs.decoder_hidden_states,
            attentions=None if outputs.encoder_attentions is None else outputs.encoder_attentions+outputs.decoder_attentions,
        )
...
class T5ForQuestionAnswering(T5PreTrainedModel):
    ...
    def forward(
        ...
        outputs = self.transformer(
            ...
            decoder_attention_mask=attention_mask if decoder_attention_mask is None else decoder_attention_mask,
            decoder_head_mask=head_mask if decoder_head_mask is None else decoder_head_mask,
            decoder_inputs_embeds=inputs_embeds if decoder_inputs_embeds is None else decoder_inputs_embeds,
            use_cache=use_cache,
            output_attentions=output_attentions,
            output_hidden_states=output_hidden_states,
            return_dict=return_dict,
        ...
        print(ss(outputs))
        sequence_output = torch.cat([outputs[0],outputs[3]],dim=1)
        print(ss(sequence_output))
